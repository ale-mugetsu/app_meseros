...
public class ListadoActivity extends Activity {
	...
	public String leer(){
		...
		HttpGet httpget = new HttpGet("http://192.168.0.117:84/api/usuario");
	...
	public ArrayList<String> obtDatosJSON(String response){
		...
		try {
			...
			for (int i=0; i<json.length();i++){
				texto = json.getJSONObject(i).getString("codigo_u") +" - "+
						json.getJSONObject(i).getString("usuario_u") +" - "+
						json.getJSONObject(i).getString("clave_u");
